    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-20">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
            ...
            <div className="space-y-8">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${whatsapp.enabled ? 'bg-[#25D366]/10 text-[#25D366]' : 'bg-secondary/10 text-secondary/40'}`}>
                    <Power className="w-6 h-6" />
                  </div>
                  <div>
                    <h3 className="text-xl font-bold text-secondary">{t('dashboard.whatsapp.enabled')}</h3>
                    <p className="text-[10px] text-muted-foreground uppercase tracking-widest">{t('dashboard.whatsapp.visibility')}</p>
                  </div>
                </div>
                <button 
                ...
                >
                  <div className={`absolute top-1.5 w-6 h-6 bg-white rounded-full transition-all shadow-sm ${whatsapp.enabled ? 'left-8' : 'left-1.5'}`} />
                </button>
              </div>

              <div className="pt-6 border-t border-secondary/5 space-y-8">
                <div className="flex justify-between items-center">
                   <h4 className="text-sm font-bold text-secondary/60">{t('dashboard.branding.general')}</h4>
          ...
          <DashboardCard padding="sm" className="bg-[#25D366]/5 border-[#25D366]/10">
            <div className="flex items-start gap-4 p-2">
              ...
              <div className="space-y-1">
                <h4 className="font-bold text-secondary text-sm">{t('dashboard.whatsapp.practices')}</h4>
        ...
        <div className="xl:col-span-5">
           <DashboardCard variant="dark" className="sticky top-8 flex flex-col items-center justify-center h-[400px] relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col items-center gap-8">
                 ...
                 <div className="text-center space-y-3">
                   <StatusBadge variant={whatsapp.enabled ? 'success' : 'neutral'}>
                     {whatsapp.enabled ? t('dashboard.whatsapp.active') : t('dashboard.whatsapp.hidden')}
                   </StatusBadge>
